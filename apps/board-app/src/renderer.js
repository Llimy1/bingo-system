// ========================================
// Board Renderer - Realtime 버전
// ========================================

const LOG = "[board-renderer]";
...
function markNumber(n) {
    ...
    if (cell) {
        ...
        markedNumbers.add(n);
        console.log(LOG, "✅ 번호 마킹:", n, "/ 총:", markedNumbers.size);
    } else {
...
// Realtime 구독 - drawn_numbers
window.api.onDrawnNumber((row) => {
    console.log(LOG, "🔴 Realtime onDrawnNumber 수신:", row);
    
    if (row && row.number) {
        markNumber(row.number);
    }
});
